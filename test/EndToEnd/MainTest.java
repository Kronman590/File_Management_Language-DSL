...
//    }
    ...
    @Test
    void testIfNotComboCond() throws IOException{
        testScript(getScriptPath("ifNotCond.fml"));

        File nondeletedFolder = new File(testFiles, "AFilledFolder");
        assertTrue(FileUtils.directoryContains(testFiles, nondeletedFolder));

        File compressedFolder = new File(testFiles, "aTestFile.txt.zip");
        assertTrue(FileUtils.directoryContains(testFiles, compressedFolder));

        File deleted = new File(testFiles, "aTestfile.txt");
        assertFalse (FileUtils.directoryContains(testFiles,deleted));

        File nondeleted = new File(testFiles, "aFilledFolder/testfile1.txt");
        assertTrue(FileUtils.directoryContains(testFiles, nondeleted));
    }
